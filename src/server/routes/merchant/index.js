...
router.post('/', async (req, res, next) => {
    try {
        ...
        if (!merchant_id || !merchant_name) {
            return res.status(400).json({ error: 'merchant_id and merchant_name are required.' });
        }
        if (typeof merchant_id !== 'string' || typeof merchant_name !== 'string') {
            return res.status(400).json({ error: 'merchant_id and merchant_name must be strings.' });
        }

        const newMerchant = await merchantCore.createMerchant({ merchant_id, merchant_name });
        ...
        // Handle specific error for unique constraint violation (merchant_id already exists)
...
router.get('/', async (req, res, next) => {
    try {
        const merchants = await merchantCore.listMerchants();
